...
public class CommentGroupActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ll_g_engagement = findViewById(R.id.ll_g_engagement);
        ll_g_skill = findViewById(R.id.ll_g_skill);
        ll_c_engagement = findViewById(R.id.ll_c_engagement);
        txt_club_count = findViewById(R.id.txt_club_count);

        ll_g_engagement.setVisibility(View.GONE);
        ll_g_skill.setVisibility(View.GONE);
        ll_c_engagement.setVisibility(View.VISIBLE);
        ...
        if (getIntent().hasExtra("gif")){
            ...
            //increaseGroupComment
            PostCount.increaseGroupComment(groupId,timeStamp,FirebaseAuth.getInstance().getCurrentUser().getUid());
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                FirebaseDatabase.getInstance().getReference().child("Groups").child(groupId).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (!snapshot.child("gIcon").getValue().toString().isEmpty()) {
                            ...
                            dp.setImageResource(R.drawable.group);
                        }
                ...
                sharePop.setOnMenuItemClickListener(item -> {
                    ...
                    if (item.getItemId() == 0){
                        ...
                        intent.putExtra("post_id",groupId);
                    ...
                    if (item.getItemId() == 1){
                        ...
                        intent.putExtra("type", "post");
                        intent.putExtra("uri", postID);
                        intent.putExtra("creater_win_id",groupId);
                        intent.putExtra("win_post_id",postID);
                        intent.putExtra("win_type","clubpost");
                        startActivity(intent);
                    }
